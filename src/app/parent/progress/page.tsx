    ...
    <div className="min-h-screen pb-10"
      ...
      <div className="bg-white/90 backdrop-blur border-b border-gray-100 px-5 pt-5 pb-4 shadow-sm">
        <div className="flex items-center gap-3 max-w-lg mx-auto">
          ...
          <h1 className="text-2xl font-black text-gray-800">📊 學習進度</h1>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-5">
          ...
          <motion.div key={child.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-yellow-600 font-bold">⭐ {child.stars} 顆星</p>
              </div>
            </div>
              ...
              return (
                <div key={cat} className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-gray-700 text-sm">{info.character} {info.label}</span>
                    <span className="font-black text-gray-800">{completed}/{levels.length}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-4 gap-1.5 mt-2">
                    {levels.map((level) => {
                      ...
                      return (
                        <div key={level.id} className={`rounded-xl p-1.5 text-center ${prog?.completed ? 'bg-green-50 border border-green-100' : 'bg-gray-50'}`}>
        ...
        {children.length === 0 && (
          <div className="bg-white rounded-3xl p-8 text-center shadow-sm border border-gray-100">
            ...
            <p className="font-black text-gray-700 mt-3 text-lg">還沒有小朋友帳號</p>
            <button onClick={() => router.push('/parent/children')}
              className="mt-3 text-indigo-600 font-bold underline hover:text-indigo-800">
              建立帳號
            </button>
          </div>
        )}
